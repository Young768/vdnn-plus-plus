...
__global__ void softmaxLossBackProp(float *y, float *SO, float *dSO,
...
__global__ void inferClass(float *O, float *IO, int batch_size,
                           int output_size) {
  int i = blockIdx.x * blockDim.x + threadIdx.x;
...
class Context {
 public:
  ...
  float learning_rate;
  float *X, *W1, *b1, *W2, *b2, *H, *Hrelu, *O, *SO, *dSO, *dO, *dW1, *db1,
      *dW2, *db2, *dH, *dHrelu;
  float *IO;
  float *y;
  float *onevec;
  ...
  // cudnnTensorDescriptor_t batchTensor, W1Tensor, b1Tensor, W2Tensor,
  // b2Tensor, HTensor, OTensor;
  ...
  Context(int input_size, int batch_size, int hidden_size, float learning_rate,
          int output_size) {
    ...
    // vdnn
    req_algo_count = 10;
    conv1fwdperf = (cudnnConvolutionFwdAlgoPerf_t *)malloc(
        req_algo_count * sizeof(cudnnConvolutionFwdAlgoPerf_t));
    conv1bwdfperf = (cudnnConvolutionBwdFilterAlgoPerf_t *)malloc(
        req_algo_count * sizeof(cudnnConvolutionBwdFilterAlgoPerf_t));
    conv1bwddperf = (cudnnConvolutionBwdDataAlgoPerf_t *)malloc(
        req_algo_count * sizeof(cudnnConvolutionBwdDataAlgoPerf_t));
    ...
    checkCUDNN(cudnnSetTensor4dDescriptor(
        inputTensor, CUDNN_TENSOR_NCHW, CUDNN_DATA_FLOAT, batch_size,
        input_feature, input_rows, input_cols));
    checkCUDNN(cudnnSetTensor4dDescriptor(
        conv1OTensor, CUDNN_TENSOR_NCHW, CUDNN_DATA_FLOAT, batch_size,
        output_feature, output_rows, output_cols));
    checkCUDNN(cudnnSetTensor4dDescriptor(conv1bTensor, CUDNN_TENSOR_NCHW,
                                          CUDNN_DATA_FLOAT, 1, output_feature,
                                          1, 1));

    checkCUDNN(cudnnSetFilter4dDescriptor(
        conv1Tensor, CUDNN_DATA_FLOAT, CUDNN_TENSOR_NCHW, output_feature,
        input_feature, filter_height, filter_width));

    checkCUDNN(cudnnCreateConvolutionDescriptor(&conv1Desc));
    int pad_h = 1, pah_w = 1, u = 1, v = 1, dilation_h = 1, dilation_w = 1;
    checkCUDNN(cudnnSetConvolution2dDescriptor(
        conv1Desc, pad_h, pah_w, u, v, dilation_h, dilation_w,
    ...
    checkCUDNN(cudnnFindConvolutionForwardAlgorithm(
        cudnnHandle, inputTensor, conv1Tensor, conv1Desc, conv1OTensor,
        req_algo_count, &ret_algo_count, conv1fwdperf));
    cout << "Printing forward conv algo perf\n";
    cout << "CUDNN_CONVOLUTION_FWD_ALGO_IMPLICIT_GEMM "
         << CUDNN_CONVOLUTION_FWD_ALGO_IMPLICIT_GEMM << endl;
    cout << "CUDNN_CONVOLUTION_FWD_ALGO_IMPLICIT_PRECOMP_GEMM "
         << CUDNN_CONVOLUTION_FWD_ALGO_IMPLICIT_PRECOMP_GEMM << endl;
    cout << "CUDNN_CONVOLUTION_FWD_ALGO_GEMM "
         << CUDNN_CONVOLUTION_FWD_ALGO_GEMM << endl;
    cout << "CUDNN_CONVOLUTION_FWD_ALGO_DIRECT "
         << CUDNN_CONVOLUTION_FWD_ALGO_DIRECT << endl;
    cout << "CUDNN_CONVOLUTION_FWD_ALGO_FFT " << CUDNN_CONVOLUTION_FWD_ALGO_FFT
         << endl;
    cout << "CUDNN_CONVOLUTION_FWD_ALGO_FFT_TILING "
         << CUDNN_CONVOLUTION_FWD_ALGO_FFT_TILING << endl;
    cout << "CUDNN_CONVOLUTION_FWD_ALGO_WINOGRAD "
         << CUDNN_CONVOLUTION_FWD_ALGO_WINOGRAD << endl;
    cout << "CUDNN_CONVOLUTION_FWD_ALGO_WINOGRAD_NONFUSED "
         << CUDNN_CONVOLUTION_FWD_ALGO_WINOGRAD_NONFUSED << endl;
    ...
    checkCUDNN(cudnnFindConvolutionBackwardFilterAlgorithm(
        cudnnHandle, inputTensor, conv1OTensor, conv1Desc, conv1Tensor,
    ...
    for (int i = 0; i < ret_algo_count; i++) {
      ...
      cout << "status: " << cudnnGetErrorString(conv1bwdfperf[i].status)
           << endl;
    ...
    checkCUDNN(cudnnFindConvolutionBackwardDataAlgorithm(
        cudnnHandle, conv1Tensor, conv1OTensor, conv1Desc, inputTensor,
    ...
    checkCudaErrors(cudaMalloc(
        &layer_input,
        batch_size * input_feature * input_rows * input_cols * sizeof(float)));
    checkCudaErrors(cudaMalloc(
        &dlayer_output,
        batch_size * output_feature * input_rows * input_cols * sizeof(float)));
    checkCudaErrors(cudaMalloc(&dW, output_feature * input_feature *
                                        filter_height * filter_width *
                                        sizeof(float)));
    checkCudaErrors(cudaMalloc(&workspace, conv1bwdfperf[0].memory));
    checkCUDNN(cudnnConvolutionBackwardFilter(
        cudnnHandle, &alpha, inputTensor, layer_input, conv1OTensor,
        dlayer_output, conv1Desc, conv1bwdfperf[0].algo, workspace,
        conv1bwdfperf[0].memory, &beta, conv1Tensor, dW));
    {
      int n;
      cout << "waiting..\n";
      cin >> n;
    ...
    exit(0);
    // checkCUDNN(cudnnGetConvolutionForwardAlgorithm(cudnnHandle, inputTensor,
    // conv1Tensor, conv1Desc, conv1OTensor,
    // 												CUDNN_CONVOLUTION_FWD_PREFER_FASTEST, 0,
    // &conv1fAlgo));
    // size_t fwd_wspace;
    // checkCUDNN(cudnnGetConvolutionForwardWorkspaceSize(cudnnHandle,
    // inputTensor, conv1Tensor, conv1Desc, conv1OTensor,
    // 													conv1fAlgo,
    // &fwd_wspace));
    ...
    // checkCudaErrors(cudaMalloc((void **)&conv1filter, filter_height *
    // filter_width * input_feature * output_feature * sizeof(float) +
    // sizeof(float)));
    // checkCudaErrors(cudaMalloc((void **)&dconv1filter, filter_height *
    // filter_width * input_feature * output_feature * sizeof(float)));
    // checkCudaErrors(cudaMalloc((void **)&conv1bias, output_feature *
    // sizeof(float)));
    // checkCudaErrors(cudaMalloc((void **)&dconv1bias, output_feature *
    // sizeof(float)));
    // fillValue<<<ceil(1.0 * output_feature / BW), BW>>>(conv1bias,
    // output_feature, 0);
    // checkCURAND(curandGenerateNormal(curandgen, conv1filter, filter_height *
    // filter_width * input_feature * output_feature + 1,
    // 								0, 1 / sqrt(filter_height * filter_width *
    // input_feature)));

    // checkCudaErrors(cudaMalloc((void **)&conv1O, batch_size * input_size_fc *
    // sizeof(float)));
    checkCudaErrors(cudaMalloc((void **)&conv1OA,
                               batch_size * input_size_fc * sizeof(float)));
    // checkCudaErrors(cudaMalloc((void **)&dconv1O, batch_size * input_size_fc
    // * sizeof(float)));
    // checkCudaErrors(cudaMalloc((void **)&dconv1OA, batch_size * input_size_fc
    // * sizeof(float)));

    // checkCUDNN(cudnnGetConvolutionBackwardFilterAlgorithm(cudnnHandle,
    // inputTensor, conv1OTensor, conv1Desc, conv1Tensor,
    ...
    // checkCudaErrors(cudaMalloc((void **)&X, batch_size * input_size *
    // sizeof(float)));
    // checkCudaErrors(cudaMalloc((void **)&W1, input_size_fc * hidden_size *
    // sizeof(float)));
    // checkCudaErrors(cudaMalloc((void **)&dW1, input_size_fc * hidden_size *
    // sizeof(float)));
    // checkCudaErrors(cudaMalloc((void **)&b1, hidden_size * sizeof(float)));
    // checkCudaErrors(cudaMalloc((void **)&db1, hidden_size * sizeof(float)));
    // checkCudaErrors(cudaMalloc((void **)&W2, hidden_size * output_size *
    // sizeof(float)));
    // checkCudaErrors(cudaMalloc((void **)&dW2, hidden_size * output_size *
    // sizeof(float)));
    // checkCudaErrors(cudaMalloc((void **)&b2, output_size * sizeof(float)));
    // checkCudaErrors(cudaMalloc((void **)&db2, output_size * sizeof(float)));
    // checkCudaErrors(cudaMalloc((void **)&H, batch_size * hidden_size *
    // sizeof(float)));
    // checkCudaErrors(cudaMalloc((void **)&dH, batch_size * hidden_size *
    // sizeof(float)));
    // checkCudaErrors(cudaMalloc((void **)&Hrelu, batch_size * hidden_size *
    // sizeof(float)));
    // checkCudaErrors(cudaMalloc((void **)&dHrelu, batch_size * hidden_size *
    // sizeof(float)));
    // checkCudaErrors(cudaMalloc((void **)&O, batch_size * output_size *
    // sizeof(float)));
    // checkCudaErrors(cudaMalloc((void **)&IO, batch_size * sizeof(float)));
    // checkCudaErrors(cudaMalloc((void **)&dO, batch_size * output_size *
    // sizeof(float)));
    // checkCudaErrors(cudaMalloc((void **)&SO, batch_size * output_size *
    // sizeof(float)));
    // checkCudaErrors(cudaMalloc((void **)&dSO, batch_size * output_size *
    // sizeof(float)));
    // checkCudaErrors(cudaMalloc((void **)&y, batch_size * sizeof(float)));
    // checkCudaErrors(cudaMalloc((void **)&onevec, batch_size *
    // sizeof(float)));
    ...
    // // checkCUDNN(cudnnSetTensor4dDescriptor(batchTensor, CUDNN_TENSOR_NCHW,
    // CUDNN_DATA_FLOAT, batch_size, rows * cols, 1, 1));

    // // // just to be able to multiply properly
    // // checkCUDNN(cudnnSetTensor4dDescriptor(W1Tensor, CUDNN_TENSOR_NCHW,
    // CUDNN_DATA_FLOAT, batch_size, hidden_size, 1, 1));
    // // checkCUDNN(cudnnSetTensor4dDescriptor(b1Tensor, CUDNN_TENSOR_NCHW,
    // CUDNN_DATA_FLOAT, 1, hidden_size, 1, 1));

    // checkCUDNN(cudnnSetTensor4dDescriptor(HTensor, CUDNN_TENSOR_NCHW,
    // CUDNN_DATA_FLOAT, batch_size, hidden_size, 1, 1));

    // // // just to be able to multiply properly
    // // checkCUDNN(cudnnSetTensor4dDescriptor(W2Tensor, CUDNN_TENSOR_NCHW,
    // CUDNN_DATA_FLOAT, hidden_size, output_size, 1, 1));
    // // checkCUDNN(cudnnSetTensor4dDescriptor(b2Tensor, CUDNN_TENSOR_NCHW,
    // CUDNN_DATA_FLOAT, 1, output_size, 1, 1));

    // checkCUDNN(cudnnSetTensor4dDescriptor(OTensor, CUDNN_TENSOR_NCHW,
    // CUDNN_DATA_FLOAT, batch_size, output_size, 1, 1));
    ...
    // checkCURAND(curandGenerateNormal(curandgen, W1, input_size_fc *
    // hidden_size, 0, 0.1));
    // checkCURAND(curandGenerateNormal(curandgen, W2, hidden_size *
    // output_size, 0, 0.1));

    // checkCURAND(curandGenerateNormal(curandgen, conv1OA, batch_size *
    // input_size_fc, 0, 0.1));
  ...
  void forwardPropagate(bool train = true) {
    ...
    // // checkCUDNN(cudnnConvolutionForward(cudnnHandle, &alpha, inputTensor,
    // X, conv1Tensor, conv1filter, conv1Desc,
    // // 									conv1fAlgo, workspace, workspace_bytes, &beta, conv1OTensor,
    // conv1O));
    ...
    // // // add bias
    // // checkCUDNN(cudnnAddTensor(cudnnHandle, &alpha, conv1bTensor,
    // conv1bias, &alpha, conv1OTensor, conv1O));

    // // // activation
    // // checkCUDNN(cudnnActivationForward(cudnnHandle, Reludesc, &alpha,
    // conv1OTensor, conv1O, &beta, conv1OTensor, conv1OA));

    float *temp = NULL;
    checkCudaErrors(cudaMallocHost((void **)&temp,
                                   batch_size * input_size_fc * sizeof(float)));
    checkCudaErrors(cudaEventRecord(start));
    checkCudaErrors(cudaMemcpyAsync(temp, conv1OA,
                                    batch_size * input_size_fc * sizeof(float),
    ...
    // // checkCudaErrors(cudaMemcpy(h_X, X, batch_size * input_size *
    ...
    // // checkCudaErrors(cudaMemcpy(h_W1, W1, input_size * hidden_size *
    // sizeof(float), cudaMemcpyDeviceToHost));
    // // cout << "W1:\n";
    ...
    // // float *h_H = (float *)malloc(batch_size * hidden_size *
    // sizeof(float));
    // // checkCudaErrors(cudaMemcpy(h_H, H, batch_size * hidden_size *
    // sizeof(float), cudaMemcpyDeviceToHost));
    // // cout << "H:\n";
    ...
    // // checkCudaErrors(cudaMemcpy(h_b1, b1, hidden_size * sizeof(float),
    // cudaMemcpyDeviceToHost));
    // // cout << "b1:\n";
    ...
    // checkCUDNN(cudnnActivationForward(cudnnHandle, Reludesc, &alpha, HTensor,
    // H, &beta, HTensor, Hrelu));

    // // checkCudaErrors(cudaMemcpy(h_H, Hrelu, batch_size * hidden_size *
    // sizeof(float), cudaMemcpyDeviceToHost));
    // // cout << "Hrelu:\n";
    // // printMatrix(h_H, batch_size, hidden_size);
    // // cout << "waiting..\n";
    ...
    // // checkCudaErrors(cudaMemcpy(h_W2, W2, hidden_size * output_size *
    // sizeof(float), cudaMemcpyDeviceToHost));
    // // cout << "W2:\n";
    ...
    // // float *h_O = (float *)malloc(batch_size * output_size *
    // sizeof(float));
    // // checkCudaErrors(cudaMemcpy(h_O, O, batch_size * output_size *
    // sizeof(float), cudaMemcpyDeviceToHost));
    // // cout << "O:\n";
    ...
    // // checkCudaErrors(cudaMemcpy(h_b2, b2, output_size * sizeof(float),
    // cudaMemcpyDeviceToHost));
    // // cout << "b2:\n";
    ...
    // if (train == false) {
    // 	inferClass<<<ceil(1.0 * batch_size / BW), BW>>>(O, IO, batch_size,
    // output_size);
    // 	cudaMemcpy(h_IO, IO, batch_size * sizeof(float),
    // cudaMemcpyDeviceToHost);
    // 	return;
    // }

    // // float *sm_test = (float *)malloc(batch_size * output_size *
    // sizeof(float));
    ...
    // // checkCudaErrors(cudaMemcpy(O, sm_test, batch_size * output_size *
    // sizeof(float), cudaMemcpyHostToDevice));
    // // checkCudaErrors(cudaMemcpy(sm_test, O, batch_size * output_size *
    // sizeof(float), cudaMemcpyDeviceToHost));
    // // cout << "O:\n";
    // // printMatrix(sm_test, batch_size, output_size);
    // // apply softmax
    // // checkCudaErrors(cudaMemcpy(h_SO, O, output_size * sizeof(float),
    // cudaMemcpyDeviceToHost));
    // // printMatrix(h_SO, batch_size, output_size);
    // checkCudaErrors(cudaDeviceSynchronize());
    // checkCUDNN(cudnnSoftmaxForward(cudnnHandle, CUDNN_SOFTMAX_ACCURATE,
    // CUDNN_SOFTMAX_MODE_INSTANCE, &alpha, OTensor, O, &beta, OTensor, SO));
    // checkCudaErrors(cudaDeviceSynchronize());
    // // checkCudaErrors(cudaMemcpy(h_SO, SO, batch_size * output_size *
    // sizeof(float), cudaMemcpyDeviceToHost));
    // // cout << "SO:\n";
    // // printMatrix(h_SO, batch_size, output_size);
    // // cout << "waiting..\n";
    ...
    // // checkCudaErrors(cudaMemcpy(h_SO, SO, batch_size * output_size *
    // sizeof(float), cudaMemcpyDeviceToHost));
    // // checkCudaErrors(cudaMemcpy(h_y, y, batch_size * sizeof(float),
    // cudaMemcpyDeviceToHost));
    // // float loss = 0;
    // // for (int i = 0; i < batch_size; i++)
    // // 	loss += -log(h_SO[i * output_size + int(h_y[i])]);
    // // // getSoftmaxLoss<<<ceil(1.0 * batch_size / BW), BW>>>(SO, y,
    // batch_size, output_size, &loss);
    // // // printf("yay");
    // // return loss;
  }
  ...
  // 	float alpha = 1.0f, beta = 0.0f;
  // 	checkCudaErrors(cudaMemset(dSO, 0, batch_size * output_size *
  // sizeof(float)));
  // 	softmaxLossBackProp<<<ceil(1.0 * batch_size / BW), BW>>>(y, SO, dSO,
  // batch_size, output_size, eps);
  // 	// int n;
  // 	// checkCudaErrors(cudaMemcpy(h_SO, dSO, batch_size * output_size *
  // sizeof(float), cudaMemcpyDeviceToHost));
  // 	// cout << "dSO:\n";
  // 	// printMatrix(h_SO, batch_size, output_size);
  // 	// cout << "waiting..\n";
  ...
  // 	// softmax backprop
  // 	checkCUDNN(cudnnSoftmaxBackward(cudnnHandle, CUDNN_SOFTMAX_ACCURATE,
  ...
  // 	// checkCudaErrors(cudaMemcpy(h_SO, dO, batch_size * output_size *
  // sizeof(float), cudaMemcpyDeviceToHost));
  // 	// cout << "dO:\n";
  // 	// printMatrix(h_SO, batch_size, output_size);
  // 	// cout << "waiting..\n";
  ...
  // 	// checkCudaErrors(cudaMemcpy(h_b2, db2, output_size * sizeof(float),
  // cudaMemcpyDeviceToHost));
  // 	// cout << "db2:\n";
  // 	// printMatrix(h_b2, 1, output_size);
  // 	// cout << "waiting..\n";
  ...
  // 	// checkCudaErrors(cudaMemcpy(h_W2, dW2, hidden_size * output_size *
  // sizeof(float), cudaMemcpyDeviceToHost));
  // 	// cout << "dW2:\n";
  // 	// printMatrix(h_W2, hidden_size, output_size);
  // 	// cout << "waiting..\n";
  ...
  // 	// float *h_H = (float *)malloc(batch_size * hidden_size *
  // sizeof(float));
  // 	// checkCudaErrors(cudaMemcpy(h_H, dHrelu, batch_size * hidden_size *
  // sizeof(float), cudaMemcpyDeviceToHost));
  // 	// cout << "dHrelu:\n";
  // 	// printMatrix(h_H, batch_size, hidden_size);
  // 	// cout << "waiting..\n";
  ...
  // 	// gradient w.r.t H
  // 	checkCUDNN(cudnnActivationBackward(cudnnHandle, Reludesc, &alpha,
  // HTensor, Hrelu, HTensor, dHrelu,
  // 										HTensor, H, &beta, HTensor,
  // dH));
  ...
  // 	checkCUDNN(cudnnActivationBackward(cudnnHandle, Reludesc, &alpha,
  // conv1OTensor, conv1OA, conv1OTensor, dconv1OA,
  // 										conv1OTensor, conv1O, &beta, conv1OTensor,
  // dconv1O));

  // 	// gradient w.r.t. conv1bias
  // 	checkCUDNN(cudnnConvolutionBackwardBias(cudnnHandle, &alpha,
  // conv1OTensor, dconv1O, &beta, conv1bTensor, dconv1bias));

  // 	// gradient w.r.t. conv1filter
  // 	checkCUDNN(cudnnConvolutionBackwardFilter(cudnnHandle, &alpha,
  // inputTensor, X, conv1OTensor, dconv1O, conv1Desc,
  // 												conv1bfAlgo, workspace,
  // workspace_bytes, &beta, conv1Tensor, dconv1filter));
  ...
  // 	// update conv1filter
  // 	checkCUBLAS(cublasSaxpy(cublasHandle, output_feature * input_feature *
  // filter_height * filter_width,
  // 							&alpha,
  // 							dconv1filter, 1,
  // 							conv1filter, 1));

  // 	// checkCudaErrors(cudaMemcpy(h_W1, W1, hidden_size * sizeof(float),
  // cudaMemcpyDeviceToHost));
  ...
  void train(int num_iter, float *train_images, float *train_labels,
             float *test_images, float *test_labels, int N) {
    // int image_size = rows * cols * channels;

    for (int iter = 0; iter < num_iter; iter++) {
      int image_id = iter % (N / batch_size);

      // checkCudaErrors(cudaMemcpy(h_W1, W1, input_size_fc * hidden_size *
      // sizeof(float), cudaMemcpyDeviceToHost));
      // checkCudaErrors(cudaMemcpy(h_W2, W2, hidden_size * output_size *
      // sizeof(float), cudaMemcpyDeviceToHost));
      // checkCudaErrors(cudaMemcpy(h_b1, b1, hidden_size * sizeof(float),
      ...
      // checkCudaErrors(cudaMemcpy(X, &train_images[image_id * batch_size *
      // input_size], input_size * batch_size * sizeof(float),
      // cudaMemcpyHostToDevice));
      // checkCudaErrors(cudaMemcpy(y, &train_labels[image_id * batch_size],
  ...
  int test(float *test_images, float *test_labels, int N) {
    ...
    while (start < N) {
      ...
      checkCudaErrors(cudaMemcpy(X, &test_images[start * input_size],
                                 input_size * size * sizeof(float),
                                 cudaMemcpyHostToDevice));
      checkCudaErrors(cudaMemcpy(y, &test_labels[start], size * sizeof(float),
                                 cudaMemcpyHostToDevice));
      this->forwardPropagate(false);
      checkCudaErrors(cudaDeviceSynchronize());
      for (int i = 0; i < size; i++) {
        if (h_IO[i] == test_labels[start + i]) count++;
        // cout << h_IO[i] << ' ';
      }
      start = start + size;
    }
...
int main() {
  ...
  // checkCudaErrors(cudaMallocHost((void **)&f_train_images, N_train *
  // input_size * sizeof(float)));
  // checkCudaErrors(cudaMallocHost((void **)&f_train_labels, N_train *
  // sizeof(float)));
  // checkCudaErrors(cudaMallocHost((void **)&f_test_images, N_test * input_size
  // * sizeof(float)));
  // checkCudaErrors(cudaMallocHost((void **)&f_test_labels, N_test *
  // sizeof(float)));
  ...
  // curandGenerator_t curandgen;
  // checkCURAND(curandCreateGeneratorHost(&curandgen,
  // CURAND_RNG_PSEUDO_DEFAULT));
  // printf("toy_train, before init:\n");
  // printMatrix(toy_train, batch_size, toy_input_size);
  // checkCURAND(curandGenerateNormal(curandgen, toy_train, batch_size *
  // toy_input_size * sizeof(float), 0, 10));
  ...
  // Context context(toy_input_size, batch_size, toy_hidden_size, toy_l_rate,
  // toy_output_size);
  // int n_iter = 100;
  // int n_rep = 10;
  // for (int i = 0; i < n_rep; i++) {
  // 	context.train(n_iter, toy_train, toy_train_labels, toy_train,
  // toy_train_labels, batch_size);
  // 	cout << context.test(toy_train, toy_train_labels, batch_size) << endl <<
  // flush;
  // }
  ...
  for (int i = 0; i < n_rep; i++) {
    context.train(n_iter, f_train_images, f_train_labels, f_test_images,
                  f_test_labels, N_train);
