...
class Context {
 public:
  ...
  Context(int input_size, int batch_size, int hidden_size, float learning_rate,
    ...
    checkCUDNN(cudnnSetPooling2dDescriptor(poolDesc, CUDNN_POOLING_MAX,
                                           CUDNN_PROPAGATE_NAN, filter_height,
                                           filter_width, pad_h, pad_w, u, v));
    ...
    checkCudaErrors(cudaMalloc((void **)&conv1OA,
                               batch_size * this->input_size * sizeof(float)));
    ...
    checkCudaErrors(
        cudaMalloc((void **)&X, batch_size * this->input_size * sizeof(float)));
    checkCudaErrors(cudaMalloc(
        (void **)&OP, batch_size * this->input_size_fc * sizeof(float)));
    ...
    checkCURAND(curandGenerateNormal(curandgen, conv1OA,
                                     batch_size * this->input_size, 0, 0.1));
  ...
  void forwardPropagate(bool train = true) {
    ...
    checkCUDNN(cudnnPoolingForward(cudnnHandle, poolDesc, &alpha, inputTensor,
                                   X, &beta, conv1OTensor, OP))
        // int n;
        // cout << "waiting..\n";
        // cin >> n;
        exit(0);
