...
void NeuralNet::getLoss(void *X, int *y, double learning_rate,
  ...
  for (int i = 0; i < num_layers; i++) {
    ...
    // 									layer_input_size[i] * data_type_size, cudaMemcpyDeviceToHost,
    // stream_memory));
    ...
    if (to_offload[i] && train == true) {
      checkPThreadErrors(pthread_create(&thread_offload_handler[i], NULL,
                                        NeuralNet::threadOffloadHandlerHelper,
                                        (void *)(&(layer_num[i]))));
    ...
    // NeuralNet::threadOffloadHandlerHelper, (void *)(&(layer_num[i]))));
  ...
  clock_gettime(CLOCK_MONOTONIC, &start_time);
  for (int i = 0; i < num_layers; i++) {
    if (to_offload[i]) checkSemaphoreErrors(sem_wait(&sem_offload_done[i]));
  }
  clock_gettime(CLOCK_MONOTONIC, &end_time);
  float lag = (end_time.tv_sec - start_time.tv_sec) * 1e3 +
              (end_time.tv_nsec - start_time.tv_nsec) * 1e-6;
