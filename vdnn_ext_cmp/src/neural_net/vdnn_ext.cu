...
// layer_input_size[layer_num] * data_type_size);
// }
...
void NeuralNet::threadOffloadHandler(int layer_num) {
  if (layer_num > 0) {
    ...
    checkCudaErrors(
        cudaMallocHost(h_layer_input[layer_num],
                       layer_input_size[layer_num] * data_type_size));

    // start offload
    checkCudaErrors(
        cudaMemcpyAsync(h_layer_input[layer_num], layer_input[layer_num],
                        layer_input_size[layer_num] * data_type_size,
                        cudaMemcpyDeviceToHost, stream_memory));
    // record event for finding point of offload completion
    checkCudaErrors(
        cudaEventRecord(event_offload_done[layer_num], stream_memory));
...
void *NeuralNet::threadCompressionScheduler(void *arg) {
  ...
  while (true) {
    ...
    while (net->compression_queue.size() == 0) {
      checkPCondErrors(pthread_cond_wait(&net->compress_job_available,
                                         &net->lock_compression_queue));
    }
...
void NeuralNet::compress(int layer_num) {
  ...
  checkCudaErrors(cudaMallocHost(
      compressed_data[layer_num].mask,
      ceil(1.0 * layer_input_size[layer_num] / COMPRESSION_BATCH_SIZE)));

  // start the threads
  pthread_t compression_threads[NUM_COMPRESSION_THREADS];
  for (int i = 0; i < NUM_COMPRESSION_THREADS; i++) {
    pthread_create(&compression_threads[i], NULL, compressThread,
                   (void *)&compress_thread_args[layer_num][i]);
  }
  for (int i = 0; i < NUM_COMPRESSION_THREADS; i++) {
...
void *compressThread(void *arg) {
  ...
  if (args->data_type == DATA_FLOAT) {
    ...
    mask_pos.slot =
        compression_metadata.start_pos[thread_num] / COMPRESSION_BATCH_SIZE;
    mask_pos.offset = 0;

    for (long i = compression_metadata.start_pos[thread_num];
         i < compression_metadata.start_pos[thread_num] +
                 compression_metadata.num_elements[thread_num];
         i++) {
      if (mask_pos.offset == 0) compressed_data.mask[mask_pos.slot] = 0;

      if (original_data[i] > 0) {
        if (compressed_data_pos.offset == -1 or
            compressed_data_pos.offset ==
                compression_metadata.slot_size[thread_num]
                                              [compressed_data_pos.slot]) {
          compressed_data_pos.slot += 1;
          checkCudaErrors(cudaMallocHost(
              (void **)&compressed_data[thread_num][compressed_data_pos.slot],
              compression_metadata.slot_size[thread_num]
                                            [compressed_data_pos.slot] *
                  sizeof(float)));
          compressed_data.slot_taken[thread_num][compressed_data_pos.slot] =
              true;
          compressed_data_pos.offset = 0;
        }
        compressed_data.mask[mask_pos.slot] =
            (compressed_data.mask[mask_pos.slot] << 1) + 1;
        ((float ***)compressed_data.data)[thread_num][compressed_data_pos.slot]
                                         [compressed_data_pos.offset] =
                                             original_data[i];
        compressed_data_pos.offset += 1;
      } else {
        compressed_data.mask[mask_pos.slot] =
            (compressed_data.mask[mask_pos.slot] << 1);
      }
      mask_pos.offset += 1;
    ...
    for (long i = compression_metadata.start_pos[thread_num];
      ...
      if (original_data[i] > 0) {
        ...
        compressed_data.mask[mask_pos.slot] =
            (compressed_data.mask[mask_pos.slot] << 1) + 1;
        ((double ***)compressed_data.data)[thread_num][compressed_data_pos.slot]
                                          [compressed_data_pos.offset] =
                                              original_data[i];
        compressed_data_pos.offset += 1;
      } else {
