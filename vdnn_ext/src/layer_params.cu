...
void ConvLayerParams::initializeValues(
  ...
  // CUDNN_CONVOLUTION_FWD_ALGO_IMPLICIT_GEMM << std::endl;
  // for (int i = 0; i < fwd_ret_count; i++) {
  // 	std::cout << i << std::endl;
  // 	std::cout << "algo: " << fwd_perf[i].algo << std::endl;
  // 	std::cout << "status: " << cudnnGetErrorString(fwd_perf[i].status) <<
  // std::endl;
  // 	std::cout << "time(ms): " << fwd_perf[i].time << std::endl;
  // 	std::cout << "memory(MB): " << fwd_perf[i].memory * 1.0 / 1024 / 1024 <<
  // std::endl;
  ...
  // CUDNN_CONVOLUTION_BWD_FILTER_ALGO_1 << std::endl;
  ...
  // 	std::cout << "memory(MB): " << bwd_filter_perf[i].memory * 1.0 / 1024 /
  // 1024 << std::endl;
  ...
  // 	std::cout << "memory(MB): " << bwd_data_perf[i].memory * 1.0 / 1024 /
  // 1024 << std::endl;
...
void ConvLayerParams::cnmemAllocDerivativesLocked(NeuralNet *net,
...
void ConvLayerParams::cnmemFreeDerivativesLocked(NeuralNet *net,
                                                 cudaStream_t stream) {
...
void FCLayerParams::cnmemAllocDerivativesLocked(NeuralNet *net,
                                                size_t data_type_size,
                                                cudaStream_t stream) {
...
void FCLayerParams::cnmemFreeDerivativesLocked(NeuralNet *net,
                                               cudaStream_t stream) {
  net->lockedcnmemFree(dW, stream);
  net->lockedcnmemFree(db, stream);
}
...
void BatchNormLayerParams::cnmemAllocDerivativesLocked(NeuralNet *net,
                                                       size_t data_type_size,
                                                       cudaStream_t stream) {
...
void BatchNormLayerParams::cnmemFreeDerivativesLocked(NeuralNet *net,
                                                      cudaStream_t stream) {
