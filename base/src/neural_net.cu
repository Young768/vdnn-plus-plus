...
NeuralNet::NeuralNet(std::vector<LayerSpecifier> &layers, DataType data_type,
                     ...
                     float init_std_dev, ConvAlgo conv_algo) {
  // create handle
  ...
  for (int i = 0; i < num_layers; i++) {
    ...
    if (layers[i].type == CONV) {
      ...
      ((FCLayerParams *)params[i])
          ->initializeValues(user_params, batch_size, data_type_size,
                             current_output_size);
  ...
  for (int i = 0; i < num_layers; i++) {
    ...
    if (layers[i].type == CONV) {
      ...
      max_input_size =
          (input_size > max_input_size) ? input_size : max_input_size;
      if (i == 0) {
  ...
  checkCudaErrors(
      cudaMalloc(&dlayer_input[0], max_input_size * data_type_size));
  checkCudaErrors(
      cudaMalloc(&dlayer_input[1], max_input_size * data_type_size));
  ...
  for (int i = 0; i < num_layers; i++) {
    if (layers[i].type == CONV) {
      ((ConvLayerParams *)params[i])
          ->getWorkspaceSize(cur_workspace_size, free_bytes, conv_algo);
  ...
  checkCudaErrors(cudaMemGetInfo(&free_bytes, &total_bytes));
  std::cout << "Free bytes just before end of NeuralNet: " << free_bytes
            << std::endl;
  {
    // int n;
    // std::cout << "Waiting..\n";
  ...
  std::cout << "Total consume(MB): "
            << (init_free_bytes - free_bytes) / (1.0 * 1024 * 1024)
            << std::endl;
...
void NeuralNet::getLoss(void *X, int *y, bool train, int *correct_count,
                        float *scalar_loss) {
  ...
  *scalar_loss = computeLoss();
  ...
  if (layer_type[num_layers - 1] == SOFTMAX) {
    ...
    // - 1];
    if (data_type == CUDNN_DATA_FLOAT) {
      checkCudaErrors(cudaMemset(dlayer_input[cur_derv], 0,
      ...
      softmaxLossBackProp<float><<<ceil(1.0 * batch_size / BW), BW>>>(
          ...
          (float *)dlayer_input[cur_derv], batch_size, num_classes,
          softmax_eps);
    } else if (data_type == CUDNN_DATA_DOUBLE) {
      checkCudaErrors(cudaMemset(dlayer_input[cur_derv], 0,
      ...
      softmaxLossBackProp<double><<<ceil(1.0 * batch_size / BW), BW>>>(
          ...
          (double *)dlayer_input[cur_derv], batch_size, num_classes,
          softmax_eps);
    }
  }
  for (int i = num_layers - 1; i >= 0; i--) {
    if (layer_type[i] == CONV) {
      ...
      checkCUDNN(cudnnConvolutionBackwardBias(
          cudnn_handle, &alpha, cur_params->output_tensor,
          dlayer_input[cur_derv], &beta, cur_params->bias_desc,
          cur_params->db));
      ...
      checkCUDNN(cudnnConvolutionBackwardFilter(
          ...
          cur_params->conv_desc, cur_params->bwd_filter_algo, workspace,
      ...
      if (i > 0)
        checkCUDNN(cudnnConvolutionBackwardData(
            ...
            workspace_size, &beta, cur_params->input_tensor,
            dlayer_input[next_derv]));
    }
      ...
      if (data_type == CUDNN_DATA_FLOAT) {
        ...
        checkCUBLAS(cublasSgemm(
            ...
            batch_size, &Salpha, (float *)dlayer_input[cur_derv],
            cur_params->C_out, (float *)one_vec, batch_size, &Sbeta,
            (float *)cur_params->db, cur_params->C_out));

        // weight backward
        checkCUBLAS(cublasSgemm(cublas_handle, CUBLAS_OP_N, CUBLAS_OP_T,
                                cur_params->C_out, cur_params->C_in, batch_size,
                                &Salpha, (float *)dlayer_input[cur_derv],
                                cur_params->C_out, (float *)layer_input[i],
                                cur_params->C_in, &Sbeta,
                                (float *)cur_params->dW, cur_params->C_out));

        // data backward
        if (i > 0)
          checkCUBLAS(cublasSgemm(
              cublas_handle, CUBLAS_OP_T, CUBLAS_OP_N, cur_params->C_in,
              batch_size, cur_params->C_out, &Salpha, (float *)cur_params->W,
              cur_params->C_out, (float *)dlayer_input[cur_derv],
              cur_params->C_out, &Sbeta, (float *)dlayer_input[next_derv],
              cur_params->C_in));
      }
        ...
        checkCUBLAS(cublasDgemm(
            cublas_handle, CUBLAS_OP_N, CUBLAS_OP_N, cur_params->C_out, 1,
            batch_size, &Dalpha, (double *)dlayer_input[cur_derv],
        ...
        checkCUBLAS(cublasDgemm(cublas_handle, CUBLAS_OP_N, CUBLAS_OP_T,
                                cur_params->C_out, cur_params->C_in, batch_size,
                                &Dalpha, (double *)dlayer_input[cur_derv],
        ...
        if (i > 0)
          checkCUBLAS(cublasDgemm(
              ...
              cur_params->C_out, (double *)dlayer_input[cur_derv],
              cur_params->C_out, &Dbeta, (double *)dlayer_input[next_derv],
              cur_params->C_in));
      }
    }
      ...
      checkCUDNN(cudnnDropoutBackward(
          cudnn_handle, cur_params->dropout_desc, cur_params->input_tensor,
          dlayer_input[cur_derv], cur_params->input_tensor,
          dlayer_input[next_derv], cur_params->reserved_space,
      ...
      checkCUDNN(cudnnBatchNormalizationBackward(
          ...
          cur_params->input_tensor, layer_input[i], cur_params->input_tensor,
          dlayer_input[cur_derv], cur_params->input_tensor,
          dlayer_input[next_derv], cur_params->sbmv_desc, cur_params->scale,
          cur_params->dscale, cur_params->dbias, cur_params->epsilon,
          cur_params->result_save_mean, cur_params->result_save_inv_var));
    }
      ...
      checkCUDNN(cudnnActivationBackward(
          cudnn_handle, cur_params->actv_desc, &alpha, cur_params->input_tensor,
          layer_input[i + 1], cur_params->input_tensor, dlayer_input[cur_derv],
          cur_params->input_tensor, layer_input[i], &beta,
          cur_params->input_tensor, dlayer_input[next_derv]));
    }
      ...
      checkCUDNN(cudnnSoftmaxBackward(
          cudnn_handle, cur_params->algo, cur_params->mode, &alpha,
          cur_params->input_tensor, layer_input[i + 1],
          cur_params->input_tensor, dlayer_input[cur_derv], &beta,
          cur_params->input_tensor, dlayer_input[next_derv]));
    }
    int temp = cur_derv;
    cur_derv = next_derv;
