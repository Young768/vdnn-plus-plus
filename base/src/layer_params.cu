...
void ConvLayerParams::initializeValues(cudnnHandle_t cudnn_handle,
                                       ConvDescriptor *user_params,
  ...
  // 	std::cout << "status: " <<
  // cudnnGetErrorString(bwd_filter_perf[i].status) << std::endl;
  // 	std::cout << "time(ms): " << bwd_filter_perf[i].time << std::endl;
  // 	std::cout << "memory(bytes): " << bwd_filter_perf[i].memory <<
  // std::endl;
...
void ConvLayerParams::getWorkspaceSize(size_t &workspace_size,
                                       size_t &free_bytes, ConvAlgo conv_algo) {
  int min_time;
  int min_index;
  size_t memory;
  size_t max_memory;
  ...
  if (conv_algo == CONV_ALGO_AUTO) {
    for (int i = 0; i < fwd_ret_count; i++) {
      if (fwd_perf[i].status == CUDNN_STATUS_SUCCESS &&
          fwd_perf[i].memory < free_bytes && fwd_perf[i].time < min_time) {
    ...
    for (int i = 0; i < bwd_filter_ret_count; i++) {
      if (bwd_filter_perf[i].status == CUDNN_STATUS_SUCCESS &&
          ...
          bwd_filter_perf[i].time < min_time) {
    ...
    // std::cout << "ConvLayerParams: workspace, filter_algo: memory: " <<
    // memory << " algo: " << bwd_filter_algo << std::endl;
    ...
    if (fwd_perf[0].status == CUDNN_STATUS_SUCCESS &&
        fwd_perf[0].memory < free_bytes) {
    ...
    if (bwd_filter_perf[0].status == CUDNN_STATUS_SUCCESS &&
        bwd_filter_perf[0].memory < free_bytes) {
      bwd_filter_algo = bwd_filter_perf[0].algo;
      // std::cout << "Free bytes " << free_bytes << std::endl;
      // std::cout << "bwd_filter_perf[i].memory " << bwd_filter_perf[i].memory
      // << std::endl;
    ...
    if (bwd_data_perf[0].status == CUDNN_STATUS_SUCCESS &&
...
void BatchNormLayerParams::allocateSpace(cudnnDataType_t data_type,
  ...
  if (data_type == CUDNN_DATA_FLOAT) {
    fillValue<float><<<ceil(1.0 * num_elements / BW), BW>>>((float *)scale,
                                                            num_elements, 1);
    fillValue<float><<<ceil(1.0 * num_elements / BW), BW>>>((float *)bias,
                                                            num_elements, 1);
  } else if (data_type == CUDNN_DATA_DOUBLE) {
    fillValue<double><<<ceil(1.0 * num_elements / BW), BW>>>((double *)scale,
                                                             num_elements, 1);
    fillValue<double><<<ceil(1.0 * num_elements / BW), BW>>>((double *)bias,
                                                             num_elements, 1);
  }
