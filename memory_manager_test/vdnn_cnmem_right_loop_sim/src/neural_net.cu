  ...
  while (true) {
    ...
    if (to_offload[0]) {
      checkCNMEMSim(cnmemMalloc(&layer_input[0],
                    ...
                    layer_input_size[0] * data_type_size, max_consume,
                    free_bytes, checkCNMEM(cnmemFinalize());
                    continue, out_of_memory);
    } else {
      checkCNMEMSim(
          cnmemMallocRight(&layer_input[0],
                           layer_input_size[0] * data_type_size, NULL),
    ...
    for (int i = 0; i < num_layers; i++) {
      ...
      if (to_offload[i + 1]) {
        checkCNMEMSim(
            cnmemMalloc(&layer_input[i + 1],
                        layer_input_size[i + 1] * data_type_size, NULL),
            layer_input_size[i + 1] * data_type_size, max_consume, free_bytes,
            break, out_of_memory);
      } else {
        checkCNMEMSimRight(
            cnmemMallocRight(&layer_input[i + 1],
                             layer_input_size[i + 1] * data_type_size, NULL),
            layer_input_size[i + 1] * data_type_size, max_consume, free_bytes,
            break, out_of_memory);
      }
      ...
      if (layer_type[i] == CONV) {
        ...
        checkCNMEMSimRight(
            cnmemMalloc(&cur_workspace, cur_workspace_size, NULL),
